...
BT.prototype = {
     ...
     */
    apply: function (btJson, useDocFrag) {
        return this[useDocFrag ? 'toDocumentFragment' : 'toHtml'](this.processBtJson(btJson));
    },
    ...
    toDocumentFragment: function (json, parent) {
        ...
        switch ((true)) {
            case Array.isArray(json):
                json.forEach(function (item) {
                    if (item !== false && item != null) {
                        this.toDocumentFragment(item, parent);
                    }
                }, this);
                break;
            case (typeof json !== 'object'):
                ...
                break;
            default:
                var node = document.createElement(json._tag || 'div');

                if (json.block && !json.elem && !json._disableDataAttrGeneration) {
                    node.setAttribute('data-block', json.block);
                }
                ...
                if (json._mixins && json.mixins.length) {
                    (initOptions || (initOptions = {})).mixins = json.mixins;
                }

                if (initOptions) {
                    node.setAttribute('data-options', JSON.stringify(initOptions));
                }

                if (json._attrs) {
                    Object.keys(json._attrs).forEach(function (attrName) {
                        node.setAttribute(attrName, json._attrs[attrName]);
                    });
                }

                if (!json._disableCssGeneration) {
                    node.classList.add((json.block) +
                        (json.view ? '_' + json.view : '') +
                        (json.elem ? '__' + json.elem : ''));

                    if (json._state) {
                        Object.keys(json._state).forEach(function (stateItem) {
                            var stateVal = json._state[stateItem];

                            if (stateVal != null && stateVal !== '' && stateVal !== false) {
                                node.classList.add('_' + stateItem + ((stateVal === true) ? '' : '_' + stateVal));
                            }
                        })
                    }

                    if (json.autoInit || (json.mixins && json.mixins.length > 0)) {
                        node.classList.add('_init');
                    }
                }
                
                if (json._content != null) {
                    this.toDocumentFragment(json._content, node);
                }
